...
$stmt = $pdo->query("SELECT w.*, u.username, u.phone FROM western_payments w JOIN users u ON w.user_id = u.id ORDER BY w.created_at DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT * FROM western_payments WHERE id = ?");
    $stmt->execute([$payment_id]);
    $payment = $stmt->fetch();
    
    if ($payment) {
        if ($action === 'approve') {
            $stmt = $pdo->prepare("UPDATE western_payments SET status = 'approved' WHERE id = ?");
            $stmt->execute([$payment_id]);
            $stmt = $pdo->prepare("UPDATE users SET withdrawal_status = 'masak_pending' WHERE id = ?");
            $stmt->execute([$payment['user_id']]);
            alert('Western Union ödemesi onaylandı!', 'success');
        } elseif ($action === 'reject') {
            $stmt = $pdo->prepare("UPDATE western_payments SET status = 'rejected' WHERE id = ?");
            $stmt->execute([$payment_id]);
            alert('Western Union ödemesi reddedildi.', 'warning');
        }
    }
    redirect('western.php');
}
?>
<!DOCTYPE html>
<html lang="tr">
<head>
    ...
    <title>Western Union - Admin Panel</title>
    ...
    <div class="flex">
        <aside class="w-64 bg-white/90 backdrop-blur min-h-screen border-r border-slate-200 fixed">
            ...
            <nav class="p-4 space-y-1">
                ...
                <a href="western.php" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-violet-500 text-white font-semibold">🌐 Western Union</a>
        ...
        <main class="flex-1 ml-64 p-8">
            ...
            <h2 class="text-3xl font-black text-slate-800 mb-2">🌐 Western Union Ödemeleri</h2>
            <p class="text-slate-500 mb-8">Western Union ödemelerini onaylayın</p>
                    ...
                    <table class="w-full">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="text-left py-4 px-6 text-slate-600 font-semibold">Çekim Tutarı</th>
                                <th class="text-left py-4 px-6 text-slate-600 font-semibold">Komisyon</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-t border-slate-100">
                                ...
                                <td class="py-4 px-6 font-bold text-purple-600 font-mono"><?= formatMoney($payment['withdrawal_amount']) ?></td>
