...
const AdminMasak = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await axios.get(`${API}/admin/masak`);
  ...
  const approvePayment = async (id) => {
    try {
      await axios.post(`${API}/admin/masak/approve/${id}`);
      toast.success('MASAK ödemesi onaylandı');
      fetchPayments();
    } catch (error) {
  ...
  const rejectPayment = async (id) => {
    try {
      await axios.post(`${API}/admin/masak/reject/${id}`);
      toast.success('MASAK ödemesi reddedildi');
      fetchPayments();
    } catch (error) {
      toast.error('İşlem başarısız');
    }
  ...
  return (
    <div className="space-y-6" data-testid="admin-masak">
      <div>
        <h1 className="font-chivo font-black text-3xl text-white mb-2">MASAK Onay</h1>
        <p className="text-zinc-400">MASAK prosedürü ödemelerini onaylayın</p>
      </div>
      ...
      <Card className="bg-zinc-900/70 border-zinc-800">
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-zinc-400">Henüz MASAK ödemesi yok</p>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-800 bg-zinc-800/50">
                    ...
                    <th className="text-left py-3 px-4 text-zinc-300 font-semibold">TRANSFER TUTARI</th>
                    <th className="text-left py-3 px-4 text-zinc-300 font-semibold">VERGİ (%15)</th>
                    <th className="text-left py-3 px-4 text-zinc-300 font-semibold">BONUS</th>
                ...
                <tbody>
                  {payments.map((payment, index) => (
                    <tr key={payment.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                      ...
                      <td className="py-3 px-4 text-gold-500 font-mono font-bold">{payment.transfer_amount?.toLocaleString('tr-TR')} TL</td>
                      <td className="py-3 px-4 text-red-400 font-mono">{payment.fee_amount?.toLocaleString('tr-TR')} TL</td>
                      <td className="py-3 px-4 text-green-400 font-mono">{payment.bonus_amount?.toLocaleString('tr-TR')} TL</td>
